...
class UserDataFormTestCase(TestCase):
    ...
    def test_wrong_email(self):
        args = ('bolek', 'BL', 'POLAK',
                'RANDOM', '512-332-543', 'Bialystok bla')
        self.assertFalse(self.helper_UserDataForm(*args))

    def test_wrong_number(self):
        args = ('bolek', 'BLO@email.com', 'POLAK',
                'RANDOM', '512-', 'Bialystok bla')
        self.assertFalse(self.helper_UserDataForm(*args))
        args = ('bolek', 'BLO@email.com', 'POLAK',
                'RANDOM', '12332-954', 'Bialystok bla')
        self.assertFalse(self.helper_UserDataForm(*args))

    def test_correct_data(self):
        args = ('bolek', 'BLO@email.com', 'POLAK',
                'RANDOM', '512-332-954', 'Bialystok bla')
        self.assertTrue(self.helper_UserDataForm(*args))
        args = ('bolek', 'BLO@email.com', 'POLAK',
                'RANDOM', '512332954', 'Bialystok bla')
        self.assertTrue(self.helper_UserDataForm(*args))
...
class registerViewTestCase(LiveServerTestCase):
    ...
    def test_correct_register(self):
        data = {'username': 'bolek', 'email': 'fake@gmail.com',
                'password1': 'useruser123', 'password2': 'useruser123'}
    ...
    def test_wrong_register(self):
        data = {'username': 'bolek', 'email': 'fake@gmail.com',
                'password1': 'useruser123', 'password2': 'useruser1'}
        self.helper_register(**data)
        user = User.objects.filter(username=data['username'])
    ...
    def test_wrong_email_register(self):
        data = {'username': 'bolek', 'email': 'fake',
                'password1': 'useruser123', 'password2': 'useruser123'}
        self.helper_register(**data)
        user = User.objects.filter(username=data['username'])
        self.assertFalse(user.exists())
...
class loginViewTestCase(LiveServerTestCase):
    ...
    #    print(User.objects.all().get(username=data['username']).username, User.objects.all().get(username=data['username']).password)
        self.helper_login(self.data['username'],
                          self.data['password'])
    ...
    def test_wrong_pass_login(self):
        self.helper_login(username=self.data['username'],
                          password='useaasdasd')
    #    print(self.driver)
